...
SCP()
{
   ...
   then
      echo "Server ${DST_SERV} is not active/discoverable!"
      exit 1
   fi
...
DATABASE_BACKUP() 
{
   ...
   PARFILE="/home/oracle/scripts/practicedir_eno_jan26/BIN/BASH/expdp_${SCHEMA}_${RUNNER}_${TS}.par"
	...
	EXPDP_LOG_PATH=${DATA_PUMP}/${EXPDP_LOG}
	DUMP_FILE="expdp_${SCHEMA}_${RUNNER}_${TS}.dmp"
	DUMP_PATH=${DATA_PUMP}/${DUMP_FILE}
...
EOF
   ...
   echo "Archiving backups..."
	cd ${DATA_PUMP}
   if ( ! tar -czvf ${DATA_PUMP}/expdp_${SCHEMA}_${RUNNER}_${TS}.tar ${DUMP_FILE} ${EXPDP_LOG} --remove-files )
   then
	...
	tar -tvf ${DATA_PUMP}/expdp_${SCHEMA}_${RUNNER}_${TS}.tar
}
...
DATABASE_IMPORT()
{
	...
	cd ${DATA_PUMP}/${SRC_DB}
	if ( ! tar -xzvf ${DATA_PUMP}/${SRC_DB}/${TAR_FILE} -C ${DATA_PUMP}/${DB_NAME} ${DUMP_FILE} )
	then
		echo "Unable to extract files!"
		exit 1
	else
		echo "Successfully extracted dump file."
		ls -ltr ${DATA_PUMP}/${DB_NAME} 
		echo
	fi
...
EOF
	...
	if [[ ! -f ${DATA_PUMP}/${DB_NAME}/${DUMP_FILE} ]]
	then
...
<<comment
	#-copy dump file
	echo "Copying dump file from ${SRC_DB} to ${DB_NAME} path..."
	if ! cp -rf ${DATA_PUMP}/${SRC_DB}/${DUMP_FILE} ${DATA_PUMP}/${DB_NAME}/${DUMP_FILE}
	then
		echo "Dump file copy FAILED!"
		exit 1
	else
		echo "Dump file copy SUCCESSFUL!"
	fi
comment
...
comment
   ...
   IMPDP_LOG="impdp_${SCHEMA}_${RUNNER}_${TS}.log"
   IMPDP_LOG_PATH=${DATA_PUMP}/${DB_NAME}/${IMPDP_LOG}
	...
	#-confirm database backup
	echo "Confirming import:"
	ls -ltr *${RUNNER}* ${DATA_PUMP}/${DB_NAME}
}
