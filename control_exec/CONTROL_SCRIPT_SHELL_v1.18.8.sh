...
#   export PEM_KEY=<full_path_to_pem_file>
...
PEM_KEY="${PEM_KEY:?'Error: PEM_KEY environment variable not set (full path to .pem file)'}"
...
database_backup)
	...
	then
		...
		echo "Calling ${SCRIPT_FUNC} function..."
		echo "..."
		DATABASE_BACKUP ${RUNNER} ${THRESHOLD} ${SCHEMA} ${DB_NAME} ${DIRECTORY} ${QUERY}
	fi
	echo
	;;
database_import)
