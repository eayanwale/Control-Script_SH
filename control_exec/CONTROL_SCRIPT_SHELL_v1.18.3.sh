...
EOF
	...
	DBCHECKLOG="/home/oracle/scripts/practicedir_eno_jan26/BIN/BASH/db_check.log"
	#-check open status
	sqlplus ${DB_USER}/${DB_PASS} <<EOF > ${DBCHECKLOG}
set echo on feedback on term on pagesize 0
select status from v\$instance;
...
comment
	...
	#-login to database and check for exported schema
	SCHEMACHECKLOG="/home/oracle/scripts/practicedir_eno_jan26/BIN/BASH/schema_check.log"	

	sqlplus ${DB_USER}/${DB_PASS} << SCHEMAEOF
spool ${SCHEMACHECKLOG}
select username from dba_users where username like '%${SCHEMA}_${RUNNER}_${TS}%';
spool off
exit
SCHEMAEOF

	echo "---------#####----------"
	if grep -q "${SCHEMA}_${RUNNER}" ${SCHEMACHECKLOG}
	then
		echo "Found imported schema: ${SCHEMA} in ${DEST_DB} as ${SCHEMA}_${RUNNER}"
	else
